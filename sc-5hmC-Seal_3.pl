...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2_removedDups"   
my $output_g = '';  ## such as "3_barcodeMark"
my $config_g = '';  ## configure file for SPRITE, such as SPRITE_Functions/config_5hmC_5rounds.txt
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 3: Identify and mark barcodes by invoking SPRITE_Functions/BarcodeIdentification.jar. 
        ...
        Usage:
               perl sc-5hmC-Seal_3.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]   [-con configureFile]
        For instance:
               nohup time  perl sc-5hmC-Seal_3.pl   -in 2_removedDups   -out 3_barcodeMark  -con  SPRITE_Functions/config_5hmC_5rounds.txt  > sc-5hmC-Seal_3.runLog.txt  2>&1    &
        ...
        -con configureFile         configure file for SPRITE.  (no default)
        -----------------------------------------------------------------------------------------------------------
...
## Version Infromation
my $version = "    The 3rd Step, version 1.2,  2024-02-15.";
...
$input_g  = '2_removedDups';    ## This is only an initialization value or suggesting value, not default value.
$output_g = '3_barcodeMark';    ## This is only an initialization value or suggesting value, not default value.
$config_g = 'SPRITE_Functions/config_5hmC_5rounds.txt';    ## This is only an initialization value or suggesting value, not default value.

## Available Arguments 
my $available = "   -version    -help   -in   -out  -con ";
my $boole = 0;
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl sc-5hmC-Seal_3.pl  -help' \n";
    exit 0;
}
...
if ( exists $args{'-con'     }   )     { $config_g = $args{'-con'     }; }else{say   "\n -con  is required.\n";   say  "\n$HELP\n";    exit 0; }
...
say    "\n\n\n\n\n\n##################################################################################################";
say    "Identify and mark barcodes ......";          
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system(" java  -jar  SPRITE_Functions/BarcodeIdentification.jar    --input1   $input_g/$end1    --input2  $input_g/$end2    --output1  $output_g/$end1    --output2  $output_g/$end2    --config  $config_g   >> $output2_g/$temp.runLog.txt  2>&1");                                                   
}
