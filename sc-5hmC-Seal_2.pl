...
my $output_g = '';  ## such as "2_removedDups"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 2: Remove PCR and optical duplicates by using clumpify.sh of BBMap.  
        
                And directly assess the quality of raw reads to identify possible experimental and sequencing errors or biases 
                by using 4 tools: FastQC, fastp, FastQ_Screen, and MultiQC.
                ...
                The versions of tools and packages are used in this script (This is shown for reference only. And do not need to match them.): 
                        Perl,   5.34.0   (perl    -v)
                        FastQC, 0.11.9   (fastqc  -v)
                        fastp,  0.23.2   (fastp   -v)
                        FastQ_Screen,  0.15.1 (fastq_screen -v)
                        BBMap,         39.01  (clumpify.sh --version) 
                        MultiQC,       1.14   (multiqc --version) 
       
        Usage:
               perl sc-5hmC-Seal_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup  time   perl sc-5hmC-Seal_2.pl   -in 1_rawFASTQ   -out 2_removedDups    > sc-5hmC-Seal_2.runLog.txt  2>&1     &      
...
## Version Infromation
my $version = "    The 2nd Step,  version 1.2,  2024-02-15. ";
...
$output_g = '2_removedDups';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl sc-5hmC-Seal_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("clumpify.sh   in=$input_g/$end1    in2=$input_g/$end2    out=$output_g/$end1  out2=$output_g/$end2  -Xmx60g  reorder=f  dedupe=t  addcount=t  subs=0  >> $output2_g/$temp.runLog  2>&1");   ## keeping the highest-quality copy, but not remove all copies of duplicates
        system( "sleep 3s" );      ## awk 'NR%4==2' in.fastq | sort -S1900G | uniq -c > output.txt                                                    
}
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("clumpify.sh    in=$input_g/$singleEnd_g[$i]    out=$output_g/$singleEnd_g[$i]   -Xmx60g   reorder=f  dedupe=t  addcount=t   subs=0  >> $output2_g/$temp.runLog.txt  2>&1"); ## keeping the highest-quality copy, but not remove all copies of duplicates
        system( "sleep 3s" );      ## awk 'NR%4==2' in.fastq | sort -S1900G | uniq -c > output.txt                  
}
}
###################################################################################################################################################################################################
