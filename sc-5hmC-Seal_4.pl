...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "3_barcodeMark"   
my $output_g = '';  ## such as "4_finalFASTQ"
{
## Help Infromation
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Get reads with full barcodes, and get ligation efficiency by invoking scripts under the folder SPRITE_Functions. 
        ...
        Usage:
               perl sc-5hmC-Seal_4.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl sc-5hmC-Seal_4.pl   -in 3_barcodeMark   -out 4_finalFASTQ    > sc-5hmC-Seal_4.runLog.txt  2>&1    &
...
## Version Infromation
my $version = "    The 4th Step, version 1.2,  2024-02-15.";
...
$input_g  = '3_barcodeMark';   ## This is only an initialization value or suggesting value, not default value.
$output_g = '4_finalFASTQ';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl sc-5hmC-Seal_4.pl  -help' \n";
    exit 0;
}
...
my $output3_g = "$output_g/ligation_efficiency";
my $output4_g = "$output_g/short_barcodes";
&myMakeDir("$output_g");
&myMakeDir("$output2_g");
&myMakeDir("$output3_g");
&myMakeDir("$output4_g");
...
say    "\n\n\n\n\n\n##################################################################################################";
say    "Get reads with full barcodes, and get ligation efficiency ......";          
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system(" python3   SPRITE_Functions/get_ligation_efficiency.py   $input_g/$end1   >  $output3_g/$temp.ligation_efficiency.txt  2>&1  ");
        system(" python3   SPRITE_Functions/get_full_barcodes.py   --r1  $input_g/$end1   >  $output2_g/$temp.runLog.txt  2>&1  ");
        sleep(5);
        system(" mv  $input_g/*.R1_short.fast*  $output4_g");
        system(" mv  $input_g/*.R1_full.fast*   $output_g");
}
