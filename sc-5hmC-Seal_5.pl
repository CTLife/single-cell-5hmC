...
my $input_g    = '';  ## such as "4_finalFASTQ"
my $output_g   = '';  ## such as "5_rawBAM"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 5: Mapping reads to the reference genome by using Bowtie2.
                And assess the quality of BAM files to identify possible mapping errors or biases by using 11 softwares:
                SAMtools, Subread utilities, FASTQC, SAMstat, qualimap, deepTools, PRESEQ, Picard, goleft, Bamtools, and MultiQC.
        ...
        Usage:
               perl sc-5hmC-Seal_5.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  
        For instance:
               nohup time  perl sc-5hmC-Seal_5.pl   -genome hg38   -in 4_finalFASTQ   -out 5_rawBAM     > sc-5hmC-Seal_5.runLog.txt  2>&1    &  
...
## Version Infromation
my $version = "    The 5th Step, version 1.2,  2024-02-15.";
...
$input_g  = '4_finalFASTQ';            ## This is only an initialization value or suggesting value, not default value.
$output_g = '5_rawBAM';                ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  sc-5hmC-Seal_5.pl  -help' \n";
    exit 0;
}
...
sub  myQC_BAM_2  {
    ...
    system("sleep 5s");
    system( "multiqc    --title goleft    --verbose  --export   --outdir $MultiQC1  --pdf          $goleft     >> $MultiQC1/MultiQC.goleft.runLog    2>&1" );
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("bowtie2     --threads $numCores_g   -q   --phred33  --trim5 11  --local   -x $Bowtie2_index_g    -1 $input_g/$end1        -2 $input_g/$end2     -S $Bowtie2_g/$temp.sam    >> $Bowtie2_g/$temp.runLog  2>&1");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("bowtie2    --threads $numCores_g   -q   --phred33  --trim5 11   --local     -x $Bowtie2_index_g    -U $input_g/$singleEnd_g[$i]       -S $Bowtie2_g/$temp.sam    >> $Bowtie2_g/$temp.runLog  2>&1");
}
