...
my $input_g    = '';  ## such as "5_rawBAM"
my $output_g   = '';  ## such as "6_finalBAM"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 6: Only the mapped reads with MAPQ>10 are retained and reads on other chromosomes, all of them are removed. 
        ...
        Usage:
               perl sc-5hmC-Seal_6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]   
        For instance:
               nohup  time  perl sc-5hmC-Seal_6.pl   -genome hg38   -in 5_rawBAM   -out 6_finalBAM       > sc-5hmC-Seal_6.PE.runLog 2>&1  &    
...
## Version Infromation
my $version = "    The 6th Step, version 1.2,  2024-02-15.";
...
$input_g  = '5_rawBAM';           ## This is only an initialization value or suggesting value, not default value.
$output_g = '6_finalBAM';         ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  sc-5hmC-Seal_6.pl  -help' \n";
    exit 0;
}
...
my $GTF = "/home/yongpeng/MyProgramFiles_02302023/14_Genomes";
if($genome_g eq "hg38"){$GTF = "$GTF/Gencode/Human_GRCh38.p13/gencode.v43.primary_assembly.annotation.gtf"; }
if($genome_g eq "mm39"){$GTF = "$GTF/Gencode/Mouse_GRCm39/gencode.vM32.primary_assembly.annotation.gtf"; }

my $BED12_g = "/home/yongpeng/MyProgramFiles_02302023/14_Genomes";
if($genome_g eq "hg38"){$BED12_g = "$BED12_g/Gencode/hg38.gencode.v43.primary_assembly.annotation.bed"; }
if($genome_g eq "mm39"){$BED12_g = "$BED12_g/Gencode/mm39.gencode.vM32.primary_assembly.annotation.bed"; }
...
sub  myQC_BAM_RNA  {
    ...
    for ( my $i=0; $i<=$#Files; $i++ ) {
        ...
        ## java -jar /home/yongpeng/MyProgramFiles_02302023/3_BAM_QC/QoRTs/QoRTs.jar  --man   QC
        &myMakeDir("$QoRTs/$temp");
        system("java  -jar  $QoRTs_g  QC    --numThreads  $numCores_g   --generatePlots   $dir1/$temp.bam   $GTF   $QoRTs/$temp   >> $QoRTs/$temp.runLog   2>&1");
        
        &myMakeDir("$RSeQC/$temp");
        system("tin.py                  --input=$dir1/$temp.bam                                                          --refgene=$BED12_g         >> $RSeQC/$temp/1-tin.runLog                   2>&1");
        ...
        system("geneBody_coverage.py    --input=$dir1/$temp.bam         --out-prefix=$RSeQC/$temp/5-geneBody_coverage    --refgene=$BED12_g         >> $RSeQC/$temp/5-geneBody_coverage.runLog     2>&1");
        system("inner_distance.py       --input-file=$dir1/$temp.bam    --out-prefix=$RSeQC/$temp/6-inner_distance       --refgene=$BED12_g         >> $RSeQC/$temp/6-inner_distance.runLog        2>&1");
        ...
        system("junction_annotation.py  --input-file=$dir1/$temp.bam    --out-prefix=$RSeQC/$temp/8-junction_annotation  --refgene=$BED12_g         >> $RSeQC/$temp/8-junction_annotation.runLog   2>&1");
        system("junction_saturation.py  --input-file=$dir1/$temp.bam    --out-prefix=$RSeQC/$temp/9-junction_saturation  --refgene=$BED12_g         >> $RSeQC/$temp/9-junction_saturation.runLog   2>&1");
        ...
        system("read_distribution.py    --input-file=$dir1/$temp.bam                                                     --refgene=$BED12_g         >> $RSeQC/$temp/11-read_distribution.runLog    2>&1");
        ...
        system("RPKM_saturation.py      --input-file=$dir1/$temp.bam    --out-prefix=$RSeQC/$temp/15-RPKM_saturation      --refgene=$BED12_g        >> $RSeQC/$temp/16-RPKM_saturation.runLog      2>&1");
       
        system("rnaseqc  $GTF   $dir1/$temp.bam   $RNA_SeQC/$temp   --rpkm  --coverage  >> $RNA_SeQC/$temp.runLog   2>&1");
    }
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("samtools  view   -h    --threads $numCores_g    -q 10   -L $genome_g.genome.chromsize.selected.bed      -o $output_g/$temp.bam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  "); 
